...
#define CO_HYSTERESIS 10.0  // Histéresis para CO
#define MIN_ALARM_TIME 1000 // Tiempo mínimo de alarma (1 segundo) - REDUCIDO
...
// Variables para persistencia de alarma
// ELIMINAR estas variables (ya no se necesitan):
// bool co_alarm_active = false;
// unsigned long alarm_start_time = 0;
// bool alarm_persistence = false;
// #define MIN_ALARM_TIME 1000
...
float test_start_flow = 0.0;  // CORREGIDO: Cambiar de unsigned long a float
...
void main(void) {
    ...
    // En el main loop, cambiar el timing:
    while(1) {
        system_millis += 125;
        ...
        // Enviar datos cada 0.25 segundos (2 × 125ms = 250ms)
        if(cycle_count >= 2) {  // CAMBIADO: de 4 a 2
            Send_Data();
            cycle_count = 0;
        }
...
void Handle_Test_Events(void) {
    ...
    if(!trigger_test && prev_test) {
        float water_used = total_flow - test_start_flow;  // CORREGIDO: Ahora ambos son float
...
void Read_Sensors(void) {
    ...
    // Detección con histéresis para llama
    ...
    // Leer MQ2 con filtrado mejorado
...
void Update_Actuators(void) {
    // SIMPLIFICADO: Respuesta inmediata sin persistencia
    fire_alarm = flame_detected || 
                 (temperature >= TEMP_ALARM_THRESHOLD) || 
                 (co_ppm >= CO_ALARM_THRESHOLD);
    
    // Activar actuadores basado en fire_alarm
    if(fire_alarm) {
        PUMP_PIN = 1;
        pump_active = true;
...
unsigned int ADC_Read(unsigned char channel) {
    ...
    ADCON0bits.GO = 1;
    while(ADCON0bits.GO);
    return ((unsigned int)ADRESH << 8) | ADRESL;  // CORREGIDO: Cast explícito
}
