...
void Read_LM35(void) {
    unsigned int adc_value = ADC_Read_Average(0, 5);
...
void Read_Flame_Sensor(void) {
    ...
    unsigned int adc_value = ADC_Read_Average(1, 5);
    float voltage = (adc_value * 5.0) / 1024.0;
...
void Read_MQ2_Sensor(void) {
    ...
    unsigned int adc_value = ADC_Read_Average(2, 5);
    float voltage = (adc_value * 5.0) / 1024.0;
    ...
    if(rs_ro_ratio < 0.1) {
        ppm = 5000.0 * powf(rs_ro_ratio,-1.0);
    }

    else if(rs_ro_ratio < 0.5) {
        ppm = 1000.0 * powf(rs_ro_ratio,-1.5);
    }

    else {
        ppm = 100.0 * powf(rs_ro_ratio,-2.0);
    }
    ...
    mq2_history_index = (mq2_history_index + 1) % 5;
...
void main(void) {
    ...
    while(1) {
        ...
        if(sensor_counter >= 4) {
            ...
            else if(calibrationCounter >= 600000) {
                Calibrate_Flame_Sensor();
            }
        }
